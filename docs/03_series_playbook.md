...
связку сотрёт. Пока серия собирается один раз, это терпимо, но если
пришлось перематчивать — ручные связки надо вписать заново. Проверять
так: после матча сверить, что `unmatched.json` пуст, а не считать, что
он таким и остался.
