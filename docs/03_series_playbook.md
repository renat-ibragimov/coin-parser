...
Скоуп он берёт **из базы**, не из `staging`:
...
Поэтому серию, залитую вчера, он подхватит сам — ничего досыпать в
staging на сервере не надо, и то, что там лежит (или не лежит), на него
не влияет. Монету с ua-coins он ищет по `price_source_links`
...
```
update-prices ok scope=6 years=3 matched=5 inserted=5 dup=0 no_quote=1 no_link=0 errors=0
```
