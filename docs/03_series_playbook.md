...
### Окно коррекции: почему день не заморожен

ua-coins не фиксирует цену дня в его начале. Он пересчитывает её в
течение дня — 10.09.2026 в 08:13 UTC монета nbu:1 стоила 349 ₴, в 08:51
уже 357 ₴, — и правит задним числом дни, которые давно закрыты (nbu:418
за 04.09: 299 ₴, когда мы прочитали, 295 ₴ неделю спустя).

Дедупликация записи ключуется на `(catalog_item_id, source, grade,
observed_at)`, цены в ключе нет. Пока это было единственным правилом,
за датой навсегда оставалось то число, которое мы прочитали первым.
Поэтому последние `REFRESH_WINDOW_DAYS = 7` дней открыты для правки:
строка внутри окна, чья цена разошлась с источником, обновляется на
месте — id и `observed_at` не меняются, коллекционерские снапшоты
(`created_by IS NOT NULL`) не трогаются вовсе. Строки старше окна не
переписываются никогда: там расхождение куда вероятнее означает кривую
выгрузку, чем правку сайта.

В отчётах обоих шагов на это заведена колонка `upd` и счётчик
`corrected`. Идемпотентный повтор даёт `inserted 0, corrected 0`.

Чинит окно ровно то, что успевает прочитать. `update-prices` берёт
годовые таблицы, где на монету одна цена и дата в заголовке колонки, —
значит он поправит день, срез которого сайт ещё отдаёт (обычно сегодня
или вчера). День глубже правится только перечитыванием истории монеты,
то есть `--step fetch-prices --refresh-prices` плюс `--step load-prices`
по серии.
...
08.09.2026»). Пишется именно эта дата, а не дата запуска: сайт вполне
может утром отдавать ещё вчерашний срез, и проставить ему сегодняшнее
число значило бы придумать котировку, которой не было.
