...
повторным `--step parse` — но не угадыванием. Неизвестный материал или
качество выбрасывает **карточку целиком**, а не оставляет поле пустым,
поэтому число карточек в `cards.json` надо сверять с `nbu_card_count`
из `series.json`: серия, собранная на 12 монет из 26, выглядит здоровой,
пока не сравнишь.

И ещё одна проверка, которую делают в базе, а не в staging, — «кого шаг
не сможет усыновить», раздел 6. Её надо пройти до записи в прод: она
ловит старые записи с коллекционными позициями, которые иначе задвоятся.

### Когда матч не сходится не по нашей вине

`unmatched.json` — не всегда наша недоработка. Бывает, что НБУ обрезал
название в собственной разметке: у `nbu:1192` во всех атрибутах страницы
стоит «…та початку формув  (н)», оборванное посреди слова, а полного
варианта на странице нет ни разу. Матчер такое не склеит и не должен:
правило префикса требует границы слова, иначе под него полезут разные
монеты с общим началом названия.

Такие случаи закрывают руками — дописывают связку прямо в
`parsed/cards.json`, подтвердив строку ua-coins по году, номиналу и
полному названию:

```json
"ua_coins": {
  "id": 2194,
  "url": "https://www.ua-coins.info/ua/list/2194-...",
  "matched_by": "manual",
  "year_used": 2017,
  "matched_at": "<UTC>",
  "manual_reason": "чем подтверждено и почему автоматом нельзя"
}
```

`matched_by: "manual"` и `manual_reason` — не украшение: по ним потом
видно, что связка поставлена человеком, а не выведена правилом.

**Что тут хрупко:** повторный `--step match` перезапишет `cards.json` и
связку сотрёт. `staging/` вдобавок в `.gitignore`, а до базы из этого
блока доезжает только `url` — он уходит в `price_source_links`, и в проде
ручная связка выглядит неотличимо от автоматической. То есть само
обоснование не живёт нигде, кроме файла, который стирается следующим
матчем.

Поэтому каждую такую связку записывают сюда, в таблицу ниже. Она и есть
то место, где решение переживает перематч: после него сверяют, что
`unmatched.json` пуст, а если нет — вписывают связки обратно отсюда.

### Ручные связки: что вписать обратно после перематча

| карточка НБУ | серия | ua-coins | чем подтверждено | почему автоматом нельзя |
|---|---|---|---|---|
| `nbu:1192` | Збройні сили України | [2194](https://www.ua-coins.info/ua/list/2194-100-rokiv-z-chasu-utvorennya-pershoho-ukrayins-koho-polku-imeni-bohdana-khmel-nyts-koho-ta-pochatku-formuvannya-ukrayins-kykh-zbroynykh-syl) | рік 2017, номінал 5 грн, повна назва «100 років з часу утворення Першого українського полку імені Богдана Хмельницького та початку формування українських збройних сил» | НБУ обрізав назву у власній розмітці — «…та початку формув  (н)», обірвано посеред слова, повного варіанту на сторінці немає жодного разу |
| `nbu:508` | Інші монети | [256](https://www.ua-coins.info/ua/list/256-mizhnarodnyy-rik-lisiv) | річна таблиця ua-coins за 2011: id 256 позначено «5 грн. (біметал)», тираж 30 тис. — точно як `material=bimetallic`, `mintage=30000` у картки | конфлікт: 256 і 1795 мають однакові назву/рік/номінал, матчер не порівнює метал |
| `nbu:509` | Інші монети | [1795](https://www.ua-coins.info/ua/list/1795-mizhnarodnyy-rik-lisiv) | річна таблиця ua-coins за 2011: id 1795 без позначки «(біметал)» (срібло за замовчуванням), тираж 3 тис. — точно як `material=silver`, `mintage=3000` у картки | той самий конфлікт 256/1795, друга сторона пари |
| `nbu:1608` | Інші монети | [2431](https://www.ua-coins.info/ua/list/2431-narodzhenyy-v-ukrayini) | річна таблиця + власна сторінка: id 2431 тираж 5 тис., ціна ~11060 ₴ — точно як `material=silver`, `mintage=5000` у картки | конфлікт 2431/2446 без позначки металу в таблиці; 2446 виявився другим, по-іншому прослагованим записом ua-coins на нейзильберовий варіант «у сувенірній упаковці» (75000 шт., «0 без упаковки» на власній сторінці) — той самий товар, що й id 2432 (уже прив'язаний до `nbu:1610` за явним хвостом назви), тобто 2446 — зайвий дубль, а не окрема позиція |

Сверить, что связки на месте:

```
python -c "import json;print([(c['source_id'],c['ua_coins']['id']) for c in json.load(open('staging/ua/<slug>/parsed/cards.json'))['cards'] if (c.get('ua_coins') or {}).get('matched_by')=='manual'])"
```
...
### Проверить ДО шага 6: кого шаг не сможет усыновить

Усыновление находит запись uCoin-эпохи ровно одним способом — по ссылке
`NBU` в `price_source_links` с номером карточки. Другого мостика нет и
быть не должно: совпадение по названию и году — это угадывание, а на
таких записях висят коллекционные позиции и расходы.

Отсюда ловушка. Если у старой записи ссылки `NBU` нет, шаг родства не
увидит и **заведёт новую запись** — рядом с прежней. Ошибкой это не
выглядит: в отчёте будет обычный `insert`, серия сойдётся по числу монет,
а на витрине окажется два экземпляра одной монеты, причём коллекция и
покупка останутся на старом, который выглядит заброшенным.

Так уже случилось: 04.09.2026 каталожный прогон завёл `nbu:1192`
(catalog_items 3117) при живой записи 819 с покупкой 2017 года за 43 ₴.
Разбирали руками — 3117 удалили с бэкапом, 819 дописали `NBU → 1192`.

Проверка перед заливкой серии, по её `series_id`:

```sql
select i.id, i.source_key, i.issue_year, i.title_original,
       (select count(*) from collection_items ci where ci.catalog_item_id = i.id) as positions
from catalog_items i
where i.series_id = <id> and i.created_by is null
  and i.source_key like 'ucoin:%'
  and not exists (select 1 from price_source_links l
                  where l.catalog_item_id = i.id and l.source = 'NBU');
```

Что вернулось — то и надо связать руками ДО шага 6: найти карточку НБУ
той же монеты и дописать мостик, после чего шаг усыновит запись, сохранив
id и всё, что на нём висит:

```sql
insert into price_source_links (catalog_item_id, source, external_id)
values (<id записи>, 'NBU', '<номер карточки НБУ>');
```

**Незакрытый остаток на 10.09.2026** — четыре украинские записи, у которых
мостика нет, все в ещё не залитых сериях. Их надо связать до того, как
эти серии пойдут по плейбуку, иначе получим ещё четыре дубля:

| id | серия | рік | позиції | назва в базі |
|---|---|---|---|---|
| 886 | Українська держава | 2022 | 1 | «Государственные символы Украины — Наш гимн» |
| 887 | Українська держава | 2022 | 1 | «Государственные символы Украины — Наш флаг» |
| 888 | Українська держава | 2022 | 1 | «Государственные символы Украины — Наш герб» |
| 923 | Флора і фауна України | 2025 | 0 | «Глубоководные обитатели — Азовское море» |

По всей базе записей uCoin-эпохи без ссылки `NBU` — 2051, но остальные
это не украинские монеты (СССР, американские квотеры, президенти), их
пайплайн НБУ не трогает вовсе. **В доведённых сериях таких нет ни одной.**
